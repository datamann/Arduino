...
void loop()
{
    if (Serial.available()){Serial.println("Serial er tilgjengelig");}
...
{
 Serial.println(mySerial.print("AT+CMGF=1\r"));    //Because we want to send the SMS in text mode
 delay(100);
 Serial.println(mySerial.println("AT + CMGS = \"+4790813585\""));//send sms message, be careful need to add a country code before the cellphone number
 delay(100);
 Serial.println(mySerial.println("A test message!"));//the content of the message
 delay(100);
 Serial.println(mySerial.println((char)26));//the ASCII code of the ctrl+z is 26
 delay(100);
 ...
 //delay(100);
   ...
   delay(100);
   mySerial.println("AT + CSCA = \"+4790813585");//recieve sms message, be careful need to add a country code before the cellphone number
   delay(100);
...
void DialVoiceCall()
{
 mySerial.println("ATD + +4790813585;");//dial the number
 delay(100);
 mySerial.println();
}
///SubmitHttpRequest()
